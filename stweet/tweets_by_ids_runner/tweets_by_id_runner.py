    ...
    def _execute_next_tweets_request(self):
        ...
        if response.is_success():
            if self.response_with_not_found(response):
                ...
                print(" total_counts, avg_likes xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx ===================================================================")
                raw_json = json.loads(user_tweet_raw[0].raw_value)
                current_likes = raw_json['legacy']['favorite_count']
                self.total_likes += current_likes
                print(self.total_likes)
                self.total_counts += len(user_tweet_raw)
                self.avg_likes = self.total_likes / self.total_counts
                print(self.total_counts)
                print(self.avg_likes)
